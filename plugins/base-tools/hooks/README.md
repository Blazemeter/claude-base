...
- `inject-ai-generated-label.sh` — `PreToolUse`/`mcp__claude_ai_Atlassian_Rovo__createJiraIssue`. Blocks the create call when the `ai_assisted` label is missing from `tool_input.additional_fields.labels` (where the MCP tool actually reads labels; legacy `tool_input.labels` / `tool_input.fields.labels` shapes are also tolerated) and tells Claude to retry with the label added under `additional_fields.labels`.
- `require-doc-task-decision.sh` — `PreToolUse`/`Bash`. The **client-side rule #4 gate**: blocks `gh pr create` on a branch that carries a real JIRA key until a doc-task **decision marker** exists at `<repo>/.claude/doc-task-decisions/<KEY>.json`. The `file-doc-task` skill writes that marker at every terminal outcome (`filed`/`updated`/`not-required`/`not-applicable`), so running the skill once — even for a refactor (which records `not-applicable`) — clears the gate. Gating the *PR* rather than a pipeline phase is deliberate: **every** workflow opens a PR, so every workflow inherits the requirement with no per-pipeline wiring.
- `nudge-doc-task-early.py` — `PreSkill`. Best-effort nudge: when a design / spec / plan / architecture skill activates, it surfaces a reminder to file the **early** DOC-ready draft (rule #4, early invocation point) so docs impact is decided during design rather than only at PR time. Advisory — never blocks. (PreSkill stdout is not a guaranteed context channel; the always-on `SessionStart` primer carries the same instruction as the reliable fallback.)

The rule hooks honour `CLAUDE_STANDARDS_SKIP=1` as an escape hatch; every skip is logged to `${CLAUDE_PLUGIN_DATA}/base-tools/standards-skip.log`.
